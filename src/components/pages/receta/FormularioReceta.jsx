...
const FormularioReceta = ({
  ...
  const cargarDatos = async () => {
    const recetaEncontrada = await obtenerRecetaAPI(id);
  ...
  return (
    <section className='container mainSection'>
      ...
      <Form
        className='my-4 sizeForm mx-auto'
        onSubmit={handleSubmit(async (valuesObject) => {
          console.log(valuesObject);
          if (id) {
            await editarRecetaAPI(valuesObject, id);
            reset();
          } else {
            await crearRecetaAPI(valuesObject);
            reset();
          }
        })}
      >
        ...
        <div className='row'>
          <Form.Group className='mb-3 col-md-6' controlId='formAutor'>
            <Form.Label className='fw-medium'>
              Autor{' '}
              <span
                className={`text-danger fw-bold ${ocultar ? 'd-none' : ''}`}
              >
                *
              </span>
            </Form.Label>
